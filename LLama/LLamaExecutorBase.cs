    ...
    public abstract class StatefulExecutorBase : ILLamaExecutor
    {
        ...
        public unsafe StatefulExecutorBase WithSessionFile(string filename)
        {
            ...
            if (_session_tokens.Count > 0)
            {
                ...
                {
                    _logger?.LogWarning($"session file has low similarity to prompt ({_n_matching_session_tokens} / {_embed_inps.Count} tokens); will mostly be reevaluated");
                }
                else
                {
                    _logger?.LogInformation($"Session file matches {_n_matching_session_tokens} / {_embed_inps.Count} tokens of prompt");
                }
            }
