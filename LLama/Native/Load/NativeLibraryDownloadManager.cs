...
{
    internal class NativeLibraryDownloadManager
    {
        /// <summary>
        ...
        public static async Task<string?> DownloadLibraryFile(NativeLibraryDownloadSettings settings, string remoteFilePath, NativeLogConfig.LLamaLogCallback? logCallback = null)
        {
            ...
            foreach(var endpoint in endpoints)
            {
                ...
                //catch(Exception ex)
                //{
                //    logCallback?.Invoke(LLamaLogLevel.Warning, $"An exception was thrown when downloading the native library file from {endpoint}: {ex.Message}");
                //}
            }
